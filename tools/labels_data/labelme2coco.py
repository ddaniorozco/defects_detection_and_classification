...
class labelme2coco(object):
    ...
    def data_transfer(self):
        for num, json_file in tqdm(list(enumerate(self.labelme_json))):
        # for num, json_file in list(enumerate(self.labelme_json)):
    ...
    def save_json(self):
        ...
        for segment in self.data_coco['annotations']:
            segment['segmentation'] = [np.array(segment['segmentation'][0]).tolist()]
        json.dump(self.data_coco, open(self.save_json_path, "w"), indent=4)


def filter_labelme_without_annotation(annotation_folder, images_folder=None):

    if images_folder is None:
        images_folder = annotation_folder
    files = glob(os.path.join(annotation_folder, '*.json'))
    for file in files:
        with open(file, 'r') as json_file:
            dict = json.load(json_file)
        if len(dict['shapes']) == 0:
...
if __name__ == "__main__":
    ...
    labelme_json = glob(os.path.join(args.labelme_images, "*.json"))
